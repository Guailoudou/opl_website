...
$allowed = ['download', 'changelog', 'docs', 'faq', 'source', 'community', 'version'];
...
foreach ($allowed as $key) {
    ...
    if ($key !== 'version' && $url !== '' && !filter_var($url, FILTER_VALIDATE_URL)) json_response(['message' => "$key 不是有效网址。"], 422);
